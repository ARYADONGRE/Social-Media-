...
def index(request):
    ...
    final_sugg=[x for x in list(new_suggestion_list) if (x not in current_user)]
    random.shuffle(final_sugg)
    
    username_profile=[]
    username_profile_list=[]

    for users in final_sugg:
...
def upload(request):
    if request.method =='POST':
        ...
        image=request.FILES.get('image_upload') #Files .get in ()
        caption=request.POST['caption']  #POST in[]
